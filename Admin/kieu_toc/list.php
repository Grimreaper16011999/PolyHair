...
?>
<h3 class="text-center text-uppercase m-2">Quản lý kiểu tóc</h3>
<a href="index.php?add" class="btn btn-success mb-4">Thêm mới kiểu tóc</a>
    ...
    <table class="table table-hover table-bordered table-light">
        ...
        foreach ($list_nv as $key => $row) : ?>
            <tbody>
                <tr>
                    <td>
                        <input type="checkbox" name="ma_kieu_toc[]" id="" value="<?= $row['ma_kieu_toc'] ?>" class="checkbox1">
                    </td>
                    <td><?= ++$key ?></td>
                    <td><?= $row['ten_kieu_toc'] ?></td>
                    <td>
                        <img src="<?= $IMG_URL ?>/mau_toc/<?= $row['hinh_anh'] ?>" alt="" width="100px">
                    </td>
                    <td><?= $row['mo_ta'] ?></td>
                    <td class="text-center">
                        <a href="index.php?edit&id=<?= $row['ma_kieu_toc'] ?>" class="btn btn-warning"><i class="far fa-edit" style="color: #fff;"></i></a>
                        <a onclick="return confirm('Bạn có chắc muốn xoá không?')" class="btn btn-danger" href="index.php?delete&ma_kieu_toc=<?= $row['ma_kieu_toc'] ?>"><i class="far fa-trash-alt"></i></a>
                    </td>
                </tr>
            </tbody>
        <?php endforeach; ?>
