...
?>
<h3 class="text-center text-uppercase m-2">Quản lý sản phẩm</h3>
<a href="index.php?add" class="btn btn-success mb-4">Thêm mới sản phẩm</a>
    ...
    <table class="table table-hover table-bordered table-light">
        ...
        foreach ($list_nv as $key => $row) : ?>
            <tbody>
                <tr>
                    <td>
                        <input type="checkbox" name="ma_san_pham[]" id="" value="<?= $row['ma_san_pham'] ?>" class="checkbox1">
                    </td>
                    <td><?= ++$key ?></td>
                    <td><?= $row['ten_san_pham'] ?></td>
                    <td>
                        <img src="<?= $IMG_URL ?>/product/<?= $row['hinh_anh'] ?>" alt="" width="100px">
                    </td>
                    <td><?= $row['gia'] ?></td>
                    <td class="text-center">
                        <a href="index.php?edit&id=<?= $row['ma_san_pham'] ?>" class="btn btn-warning"><i class="far fa-edit" style="color: #fff;"></i></a>
                        <a onclick="return confirm('Bạn có chắc muốn xoá không?')" class="btn btn-danger" href="index.php?delete&ma_san_pham=<?= $row['ma_san_pham'] ?>"><i class="far fa-trash-alt"></i></a>
                    </td>
                </tr>
            </tbody>
        <?php endforeach; ?>
